...
// ********** Code for ReadFromRest **************
function ReadFromRest(_nocheintext) {
  this._nocheintext = _nocheintext;
}
ReadFromRest.prototype.run = function() {
  this._setEventHandlers();
  this._readFromRest_write(this._nocheintext);
}
ReadFromRest.prototype._setEventHandlers = function() {
  var $this = this; // closure support
  get$$document().query("#clicker").get$on().get$click().add$1((function (event) {
    var message = get$$document().query("#status").get$innerHTML();
    message = $add$(message, ".");
    $this._readFromRest_write(message);
  })
  );
}
ReadFromRest.prototype._readFromRest_write = function(message) {
  get$$document().query("#status").set$innerHTML(message);
}
// ********** Code for top level **************
//  ********** Library SampleEventHandling **************
// ********** Code for top level **************
function main() {
  new ReadFromRest("17").run();
}
// 199 dynamic types.
// 217 types
// 19 !leaf
